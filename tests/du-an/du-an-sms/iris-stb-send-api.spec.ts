...
test.describe('Dự án SMS môi trường dev', () => {
    ...
    test("STB - Send", async ({ sendSTBSendingAPI, generateRandomData, envEnvironmentVariables }) => {
        ...
        // ---------------- TESTCASE 02 ----------------
        await test.step(`Testcase 02: STB Send - Thành công`, async () => {

            for (let i = 1; i <= 1; i++) {
                const { smsId, message } = generateRandomData();
                console.log(`🟢 Testcase 02: Mã 0 - Thành công => Gửi tin lần ${i} có: \n 👉 serviceId: ${serviceID},\n 👉 ${smsId},\n 👉 message: ${message},\n 👉 userId: ${userId}, \n 👉 password: ${passWord}`);
        ...
        // ---------------- TESTCASE 03 ----------------
        await test.step(`Testcase 03: STB Send - Tài khoản/ Mật khẩu không đúng`, async () => {
        ...
        // ---------------- TESTCASE 04 ----------------
        await test.step(`Testcase 04: STB Send -  Tin nhắn bị lặp`, async () => {
        ...
        // ---------------- TESTCASE 04 ----------------
        await test.step(`Testcase 05: STB Send -  Độ dài tin nhắn không hợp lệ`, async () => {
