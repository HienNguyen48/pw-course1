...
test.skip("Bài học 2: Product page", async ({ page }) => {
    const number1 = 3;
    const number2 = 2;
    const number3 = 1;
    ...
    await test.step.skip('Bài học 2: product page', async () => {
        await productPageOne.navigateProductPage();
        await productPageOne.gotoPage("product");
    });

    await test.step.skip("Thêm SP 1: 2 sản phẩm vào giỏ hàng", async () => {
        await productPageOne.AddProductToCart("Product 1", number1);
    });

    await test.step.skip("Thêm SP 2: 3 sản phẩm vào giỏ hàng", async () => {
        await productPageOne.AddProductToCart("Product 2", number2);
    });

    await test.step.skip("Thêm SP 3: 1 sản phẩm vào giỏ hàng", async () => {
        await productPageOne.AddProductToCart("Product 3", number3);
    });

    //Kiểm tra bảng hiển thị 
    await test.step.skip("verify thông tin", async () => {
    ...
    await test.step.skip("Tính tổng của all sản phẩm", async() => {
